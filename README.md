...
compass, weather, and comprehensive sensor data capture.

## ✨ Features

### 📷 Camera Capabilities

- High-quality photo capture with multiple filters
  (Normal, B&W, Sepia, Vintage, Vivid)
- Digital zoom (1x - 3x)
...
├── js/
│   ├── main.js              # App bootstrap & wiring entry point
...
│       ├── camera/          # Capture pipeline, audio, overlays/
│       ├── sensors/         # GPS, orientation, geocoding, weather
│       ├── gallery/         # Gallery render, viewer, bulk-actions
│       ├── storage/
│       │   └── photoDb.js   # IndexedDB operations
│       ├── pwa/             # Service worker registration
│       ├── ui/              # Viewport, orientation, wake lock, feature UI
│       ├── wiring/          # Event binding (capture, gallery, menus, verify…)
│       ├── features/        # Focus, white balance, HDR, QR, comparison,
│       │                    #   photocode (consistency checksum), metadata/
│       └── core/            # i18n, settings, status, utils
...
- 🧹 Unified HDR state ownership, removed production `alert()` and
  debug backdoors
...
**Version 8.0.3** • Built with ❤️ for professional survey work • 2026
